...
<head>
    ...
    <title>Atualizar Proventos</title>
</head>
<body class="bg-dark">
    <!-- inicio cabecalho -->
	...
	<div class="alert text-center alert-danger"> ${erro}</div>
        ...
        <h2>Edição de Proventos</h2>
        <form class="form needs-validation" action="cadProv" method="post" novalidate>
            <div class="container new-form text-light">
            
				<input type="hidden" value="atualizar" name="acao"/>
				<input type="hidden" value="${cadProv.cdProvento}" name="codigo"/>

                <input type="hidden"  name="cpf" class="form-control" aria-describedby="emailHelp" placeholder="Ex:41558965430"  maxlength="11" minlength="11" required autofocus value="${cadProv.cpf}">

                <div class="mb-3">
                    <label for="dsProventos" class="form-label">Tipo de Proventos</label>
                    <input type="text" name="dsProventos" class="form-control" placeholder="Ex: Mercado, Mêcanica, Peça e etc." maxlength="30" required value="${cadProv.dsProventos}">
                    <i class="fa-solid fa-user-tie icon"></i>
                </div>

                <div class="mb-3">
                    <label for="vlProventos" class="form-label">Valor</label>
                    <input type="number" name="vlProventos" class="form-control" placeholder="Ex: 1588.60" maxlength="9" required value="${cadProv.vlProventos}">
                    <i class="fa-solid fa-user icon"></i>
                </div>

                <div class="mb-3">
                    <label for="dtProventos" class="form-label">Data do Recebimento</label>
                    <input type="date" class="form-control" name="dtProventos" required value="${cadProv.dtProventos}">
                    <i class="fa-solid fa-cake-candles icon"></i>
                </div>

                <div class="mb-3">
                    <label for="dsAdicionais" class="form-label">Informações Adiconais</label>
                    <input type="text" class="form-control" name="dsAdicionais" placeholder="EX: gastos com carro" maxlength="30" required value="${cadProv.dsAdicionais}">
                    <i class="fa-solid fa-key icon"></i>
                </div>   

                <button type="submit" class="btn btn-primary">Salvar Mudanças</button>
                <a href="cadProv?acao=listar" class="btn btn-danger">Cancelar</a>
